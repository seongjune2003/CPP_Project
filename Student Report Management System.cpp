...
class student {
    int rollno;         // 학생번호
    char name[50];      // 이름
    int eng_marks, math_marks, sci_marks, lang2_marks, cs_marks;        // 점수들
    double average;
    char grade;
...
void student::calculate() {
    average = (eng_marks + math_marks + sci_marks + lang2_marks + cs_marks) / 5.0;      // 5.0으로 나눠서 float으로 결정됨
    if (average >= 90)
        grade = 'A';
    else if (average >= 75)
        grade = 'B';
...
void student::getdata() {
    ...
    cin >> rollno;
    cout << "\n\nEnter student name: ";
    cin.ignore();   // 이전의 버퍼 지움    cin과 getline을 같이 사용할 때 cin.ignore()이 필요함   cin은 \n으로 인한 공백을 처리안하고 getline은 이 공백을 가져오기 때문에 cin.ignore로 없애줘야한다. 
    cin.getline(name, 50);
...
int main() {
    char ch = '0';      // 초기화 해주라해서 해줬음.
    cout << setprecision(2);    // 출력범위 지정.
    do {
        char ch;    // 선택
        int num;    // roll number
        system("cls");
        ...
        cin >> ch;
        system("cls");  // system("cls")로 콘솔창의 글들을 지움
        ...
        case '6': cout << "Existing, Thank you!"; exit(0);
        }
    } while (ch != '6');
    return 0;
}
...
void create_student() {
    student stud;
    ofstream oFile;     // ofstream은 파일을 쓰기, ifstream은 파일을 읽기
...
void display_all() {
    ...
    while (inFile.read(reinterpret_cast<char*> (&stud), sizeof(student))) {
        stud.showdata();    // st.showdata()라고 되있는데 확인좀
        cout << "\n\n==========================\n";
    }
    inFile.close();
    cin.ignore();
...
void change_student(int n) {
    ...
    while (!f1.eof() && found == false) {   // 끝에 도달하지 않았고 found가 false면
